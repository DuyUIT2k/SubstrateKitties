...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
		...
		pub fn mint(
			...
			// Performs this operation first as it may fail, use has enough kitties or not
			let new_cnt = Self::kitty_cnt().checked_add(1)
			.ok_or(<Error<T>>::KittyCntOverflow)?;
		
			// Performs this operation first because as it may fail, try to add a kittie id to owner
